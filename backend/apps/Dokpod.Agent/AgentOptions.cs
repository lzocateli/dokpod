    ...
    bool RunOnce)
{
    ...
    {
        var defaultValue = OperatingSystem.IsWindows() ? @"\\.\pipe\docker_engine" : "/run/docker.sock";
        var value = Environment.GetEnvironmentVariable("DOKPOD_AGENT_DOCKER_SOCKET") ?? defaultValue;

        if (OperatingSystem.IsWindows())
        {
            if (!value.StartsWith(@"\\.\pipe\", StringComparison.OrdinalIgnoreCase) || value.Length == @"\\.\pipe\".Length)
            {
                throw new InvalidOperationException("DOKPOD_AGENT_DOCKER_SOCKET must be a Windows named pipe path.");
            }

            return value;
        }

        return GetAbsolutePath("DOKPOD_AGENT_DOCKER_SOCKET", value);
    }
